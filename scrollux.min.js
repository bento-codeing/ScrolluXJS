class Scrollux{constructor(custom){this.$body=$('body');this.$section=$('section');this.$divanchor=null;this.navWidth=$(window).width();this.navHeight=$(window).height();this.$navigator=null;this.keysPx=null;this.currentPage=null;this.scrollbar=null;this.counter=1;this.anchors=[];this.defaults={overflowY:"hidden",overflowX:"hidden",ringListColor:"#FFF",ringListSize:"9px",ringActualSessionColor:"#FFF",ringHoverColor:"#FFF",setTimeAnimation:1000};this.settings=$.extend({},this.defaults,custom);this.init()}
init(){var i=this.$section.length;if(i>0){var settings=this.settings;this.navigatorClient();this.keysSettingsPx();this.findSACA(i);this.createAside(i);this.displayCSS(settings.overflowY,settings.overflowX,settings.ringListColor,settings.ringListSize);this.$divanchor=$('.divanchor');this.hoverList(1,settings.ringActualSessionColor);this.fcurrentPage(i);this.keymap(i,settings.setTimeAnimation);this.scrollWinY(i,settings.setTimeAnimation);this.anchorClick();this.mouseHover(settings.ringHoverColor,settings.ringActualSessionColor)}else{console.log("You don't have any DOM section in your body !")}}
